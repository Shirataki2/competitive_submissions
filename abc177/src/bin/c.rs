...
fn main() {
    ...
    mod_int::set_mod_int(MOD);
    input!(n: usize, a: [u64; n]);
    let a = a.iter().map(|&x| mod_int::ModInt::new(x))
                    .collect::<Vec<mod_int::ModInt>>();
    let mut s1 = a.iter().fold(mod_int::ModInt::new(0u64), |acc, x| acc + *x);
    s1 = s1 * s1;
    let s2 = a.iter().fold(mod_int::ModInt::new(0u64), |acc, x| acc + *x * *x);
    let ans = (s1 - s2) / mod_int::ModInt::new(2u64);
    println!("{}", ans.value());
}
