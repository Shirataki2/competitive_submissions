...
fn main() {
    ...
    let a = a.iter().map(|&v| ntt::ModInt::<ntt::Mod998244353>::new(v)).collect::<Vec<_>>();
    let f = fps::FPS::new(a);
    let g = f.inv();
    let ans = g.values().iter().map(|&v| v.to_string()).collect::<Vec<String>>().join(" ");
    outln!(io => "{}", ans);
}
...
pub mod fps {
    ...
    impl <M: ModuloPrimitive> FPS<M> {
        ...
        pub fn exp_degree(&self, deg: i64) -> Self {
            ...
            (0..).map(|i| 1 << i).take_while(|&i| i < deg as usize).for_each(|i| {
                v = (v.clone() * self.head(i << 1) - v.log_degree(1 << 1) + one).head(i << 1);
            });
            v.0.resize(deg as usize, ModInt::zero());
            v
        }
